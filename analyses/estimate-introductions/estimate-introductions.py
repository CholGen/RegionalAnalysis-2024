...
def get_introductions( tree, field, interest, inclusion_dict, root_height=0 ):
    return_list = list()
    ...
    for i in tree.preorder_node_iter( filter_fn=_identity_func ):
        ...
        if len( children ) > 0:
            ...
            try:
                ...
                node_dict["earliest_date_ncg"] = -1
                node_dict["latest_date_ncg"] = -1
            te_list = [i.split( "|" )[3] for i in node_dict["children.names"]]
            node_dict["te"] = Counter( te_list ).most_common()[0][0]
        elif i.is_leaf():
            node_dict["children.names"] = i.taxon.label
            node_dict["te"] = i.taxon.label.split( "|" )[3]
...
def parse_lineages( trees, burnin, trait, states, metadata, output ):
    ...
    for tree_idx, t in enumerate( yielder ):
        if tree_idx == 0:
            recent_date = _toYearFraction( md.loc[md["taxa"].isin([i.label for i in t.taxon_namespace]),"collection_date"].max() )
        if tree_idx < burnin:
            continue
        ...
        for loc in states:
            temp_df = get_introductions( t, trait, loc, inclusion_dict=workshop_dict, root_height=root_height )
...
if __name__ == "__main__":
    ...
    parser.add_argument( "--metadata", help="path to metadata file. Must be in csv format." )
